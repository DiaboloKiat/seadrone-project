...
class MyServer
{
  private:
    ...
    bool serviceCb(apriltag::my_service::Request &req, apriltag::my_service::Response &res)
    {
    ...
    // https://docs.opencv.org/2.4/doc/apriltags/objdetect/cascade_classifier/cascade_classifier.html
  ...
  public:
    MyServer(): it_(nh_)
    {
      ...
      package_path_ = ros::package::getPath("apriltag");
      ...
      {
        ROS_ERROR("Error loading config file, end the process..."); 
        ros::shutdown();
      }
      else 
      {
        ROS_INFO("Successfully loading config file, start to detect face...");
      }
      image_sub_ = it_.subscribe("/seadrone_image/image", 1, &MyServer::imageCb, this);
      service_ = nh_.advertiseService("my_service", &MyServer::serviceCb, this);
    }
    ~MyServer() {}
};
